...
#SBATCH --output=/home/mnguest12/slurm/sweep_actPosW.%j.out
#SBATCH --error=/home/mnguest12/slurm/sweep_actPosW.%j.err
...
# ✅ sweep: act-pos-weight (baseline = 10.0)
ACT_POS_WEIGHTS=(2.0 5.0 10.0 20.0)

for W in "${ACT_POS_WEIGHTS[@]}"; do
  TAG="actPosW_${W}"
  ...
  echo "=============================="
  echo "Starting run with act-pos-weight=${W}"
...
# (optional) also set in config, in case code reads it from there
cfg.setdefault("training", {})
cfg["training"]["act_pos_weight"] = posW
...
print(f"[sweep] training.act_pos_weight = {cfg['training'].get('act_pos_weight')}")
PY
