...
echo "========================================================"
echo "[INFO] Starting CT pipeline (FDK → µ→HU (crop) → TS → Overlay)"
echo "========================================================"
...
# === 4) µ → HU (Anchors + Keep-Box + physisches Cropping + iso 1.0 mm + Preview) ===
echo "[STEP 2] µ→HU calibration + air-removal + keep-box CROPPING ..."
python mu_to_hu_and_preview_3.py \
  --in  "${RECON_RAS}" \
  --out "${HU_NII}" \
