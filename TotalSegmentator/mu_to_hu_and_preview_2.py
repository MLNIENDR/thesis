...
def apply_air_margins_mm(hu, aff, margins):
    ...
    Diese Randbänder werden auf -1024 gesetzt.
    """
    dx,dy,dz = float(aff[0,0]), float(aff[1,1]), float(aff[2,2])
    X,Y,Z = hu.shape
...
def calibrate_to_hu(in_path, out_path, anchors, radius_mm=8.0,
    ...
    hu_i16 = np.clip(hu, hu_min, hu_max).astype(np.int16)
    out_img = nib.Nifti1Image(hu_i16, aff)
    ...
    if resample_mm and resample_mm > 0:
        ...
        out_float = nib.Nifti1Image(hu_i16.astype(np.float32), aff)
